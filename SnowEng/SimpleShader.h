...
/* ----------------------------------------------------------------------------------------- *
 * Used by simple shaders to store information about specific variables in constant buffers. *
 * ----------------------------------------------------------------------------------------- */
...
/* ----------------------------------------------------------------------------------------------------------- *
 * Contains information about a specific constant buffer in a shader, as well as the local data buffer for it. *
 * ----------------------------------------------------------------------------------------------------------- */
...
{
  public:
  ISimpleShader(ID3D11Device* device, ID3D11DeviceContext* context, bool ShaderValid = true);
  virtual ~ISimpleShader();

  //Initialization method (since it is not possible to invoke derived class overrides in the base class constructor)
  bool LoadShaderFile(LPCWSTR shaderFile);

  //Simple helpers:
  bool IsShaderValid() {return shaderValid;}

  //Activate shader and copy data:
  void SetShader();
