 ...
 * ============================== */
  ...
  * -------------------------------------------- */
ISimpleShader::ISimpleShader(ID3D11Device* Device, ID3D11DeviceContext* context, bool ShaderValid)
{
  //Save the device.
...
{
  //Derived class destructors will call this class's clean-up-method.
  if(shaderBlob)
    shaderBlob->Release();
}

/* ----------------------------------------------- *
 * Cleans up the variable table and buffers.       * 
 * Some things will be handled by derived classes. *
 * ----------------------------------------------- */
 ...
 *                                                                                   *
 * Receives: "shaderFile" = A "wide string" specifying the compiled shader to load.  *
 *                                                                                   *
...
bool ISimpleShader::LoadShaderFile(LPCWSTR shaderFile)
{
  ...
  /* Create the shader.
   * Calls an overloaded version of this abstract method in the appropriate child class. */
  shaderValid = CreateShader(shaderBlob);
  if(!shaderValid)
    return false;

  //Set up shader reflection to get information about this shader and its variables, buffers etc.
  ...
  for(unsigned int r = 0; r < resourceCount; ++r)
  {
    ...
    switch(resourceDesc.Type)
    {
      case D3D_SIT_TEXTURE: //A texture resource
      {
        ...
        srv->Index = static_cast<UINT>(shaderResourceViews.size());	//Raw index
      ...
      case D3D_SIT_SAMPLER: //A sampler resource
      {
        ...
        samp->Index = static_cast<UINT>(samplerStates.size()); //Raw index
  ...
  for(unsigned int b = 0; b < constantBufferCount; ++b)
  {
    ...
    //Get the description of the resource binding, so we know exactly how it's bound in the shader.
    ...
    device->CreateBuffer(&newBuffDesc, nullptr, &constantBuffers[b].ConstantBuffer);
...
/* ------------------------------------------------------------------------------------------ *
 * Helper for looking up a variable by name and also verifying that it is the requested size. *
 *                                                                                            *
 * Receives:                                                                                  *
...
{
  //Look for the key.
  std::unordered_map<std::string, SimpleShaderVariable>::iterator result = varTable.find(name);
...
{
  //Look for the key.
  std::unordered_map<std::string, SimpleConstantBuffer*>::iterator result = cbTable.find(name);
...
void ISimpleShader::SetShader()
{
  ...
  //Set the shader and any relevant constant buffers, which is an overloaded method in a subclass.
  SetShaderAndCBs();
}

/* ------------------------------------------------------------------ *
 * Copies the relevant data to all of this shader's constant buffers. *
 * To just copy one buffer, use "CopyBufferData()".                   * 
 * ------------------------------------------------------------------ */
void ISimpleShader::CopyAllBufferData()
{
  ...
  for(unsigned int i = 0; i < constantBufferCount; ++i)
  {
    ...
    deviceContext->UpdateSubresource(constantBuffers[i].ConstantBuffer, 0, nullptr, constantBuffers[i].LocalDataBuffer, 0, 0);
  }
}
 ...
 *                                                              * 
 * NOTE: The "index" of the buffer might NOT be the same as     *
 *       its register, especially if you have buffers bound     *
 *       to non-sequential registers!                           *
 * ------------------------------------------------------------ */
void ISimpleShader::CopyBufferData(unsigned int index)
{
  ...
  SimpleConstantBuffer* cb = &constantBuffers[index];
  if(!cb) 
    return;

  //Copy the data and disappear like a ninja.
  deviceContext->UpdateSubresource(cb->ConstantBuffer, 0, nullptr, cb->LocalDataBuffer, 0, 0);
}
...
void ISimpleShader::CopyBufferData(std::string bufferName)
{
  ...
  //Check for the buffer.
  SimpleConstantBuffer* cb = FindConstantBuffer(bufferName);
  if(!cb) 
    return;
 ...
 * Sets INTEGER data. *
 * ------------------ */
bool ISimpleShader::SetInt(std::string name, int data) {return SetData(name, (void*)(&data), sizeof(int));}

/* -------------------------------------------------------- *
 *  Sets a FLOAT-variable by name in the local data buffer. *
 * -------------------------------------------------------- */
bool ISimpleShader::SetFloat(std::string name, float data) {return SetData(name, (void*)(&data), sizeof(float));}

/* --------------------------------------------------------- *
 *  Sets a FLOAT2-variable by name in the local data buffer. *
 * --------------------------------------------------------- */
bool ISimpleShader::SetFloat2(std::string name, const float data[2]) {return SetData(name, (void*)data, sizeof(float) * 2);}

/* --------------------------------------------------------- *
 *  Sets a FLOAT2-variable by name in the local data buffer. *
 * --------------------------------------------------------- */
bool ISimpleShader::SetFloat2(std::string name, const DirectX::XMFLOAT2 data) {return SetData(name, &data, sizeof(float) * 2);}

/* --------------------------------------------------------- *
 *  Sets a FLOAT3-variable by name in the local data buffer. *
 * --------------------------------------------------------- */
bool ISimpleShader::SetFloat3(std::string name, const float data[3]) {return SetData(name, (void*)data, sizeof(float) * 3);}

/* --------------------------------------------------------- *
 *  Sets a FLOAT3-variable by name in the local data buffer. *
 * --------------------------------------------------------- */
bool ISimpleShader::SetFloat3(std::string name, const DirectX::XMFLOAT3 data) {return SetData(name, &data, sizeof(float) * 3);}

/* --------------------------------------------------------- *
 *  Sets a FLOAT4-variable by name in the local data buffer. *
 * --------------------------------------------------------- */
bool ISimpleShader::SetFloat4(std::string name, const float data[4]) {return SetData(name, (void*)data, sizeof(float) * 4);}

/* --------------------------------------------------------- *
 *  Sets a FLOAT4-variable by name in the local data buffer. *
 * --------------------------------------------------------- */
bool ISimpleShader::SetFloat4(std::string name, const DirectX::XMFLOAT4 data) {return SetData(name, &data, sizeof(float) * 4);}

/* --------------------------------------------------------------- *
 *  Sets a MATRIX (4x4) variable by name in the local data buffer. *
 * --------------------------------------------------------------- */
bool ISimpleShader::SetMatrix4x4(std::string name, const float data[16]) {return SetData(name, (void*)data, sizeof(float) * 16);}

/* --------------------------------------------------------------- *
 *  Sets a MATRIX (4x4) variable by name in the local data buffer. *
 * --------------------------------------------------------------- */
bool ISimpleShader::SetMatrix4x4(std::string name, const DirectX::XMFLOAT4X4 data) {return SetData(name, &data, sizeof(float) * 16);}

/* -------------------------------------------------------------- *
 * Gets information about a shader variable, providing it exists. *
 * -------------------------------------------------------------- */
const SimpleShaderVariable* ISimpleShader::GetVariableInfo(std::string name) {return FindVariable(name, -1);}

/* ------------------------------------------------------ *
 * Gets information about an SRV in the shader (or null). *
 *                                                        *
 * Receives:                                              *
...
{
  //Look for the key.
  std::unordered_map<std::string, SimpleSRV*>::iterator result = textureTable.find(name);
...
/* --------------------------------------------------------- *
 * Gets information about a sampler in the shader (or null). *
 *                                                           *
 * Receives:                                                 * 
...
{
  //Look for the key.
  std::unordered_map<std::string, SimpleSampler*>::iterator result = samplerTable.find(name);
...
/* -------------------------------------------------------------------------- *
 * Gets information about a particular constant buffer by name, if it exists. *
 * -------------------------------------------------------------------------- */
const SimpleConstantBuffer* ISimpleShader::GetBufferInfo(std::string name) {return FindConstantBuffer(name);}

/* ---------------------------------------------------- *
 * Gets information about a particular constant buffer  *
 *                                                      *
 * Receives:                                            *
 * index = The index of the sampler                     *
 * ---------------------------------------------------- */
 ...
 * ================================ */
  ...
  * -------------------------------- */
SimpleVertexShader::SimpleVertexShader(ID3D11Device* device, ID3D11DeviceContext* context) : ISimpleShader(device, context)
{
  //Set to zero to successfully trigger the input layout creation during "LoadShader()".
  inputLayout = nullptr;
  shader = nullptr;
 ...
 * ------------------------------------------------------- */
SimpleVertexShader::SimpleVertexShader(ID3D11Device* device, ID3D11DeviceContext* context, ID3D11InputLayout* InputLayout, bool PerInstanceCompatible) : ISimpleShader(device, context)
{
  //Save the custom input layout.
  inputLayout = InputLayout;
  shader = nullptr;

  //Unable to determine from input layout - requiring user to tell.
  perInstanceCompatible = PerInstanceCompatible;
}

/* ----------------------------------------------------------- *
 * Destructor                                                  *
 * Clean up actual shader (base will be called automatically). *
 * ----------------------------------------------------------- */
...
{
  //Clean up first; in the event this method is called more than once on the same object.
  CleanUp();
  ...
  /* Vertex shader was created successfully, so we now use the shader code to re-reflect and create an input layout that matches what the vertex shader expects. 
   * Code adapted from: https://takinginitiative.wordpress.com/2011/12/11/directx-1011-basic-shader-reflection-automatic-input-layout-creation/ */
  ...
  HRESULT hr = device->CreateInputLayout(&inputLayoutDesc[0], static_cast<UINT>(inputLayoutDesc.size()), shaderBlob->GetBufferPointer(),
...
/* ------------------------------------------------------------------------------------- *
 * Sets the vertex shader, input layout and constant buffers for future DirectX drawing. *
 * ------------------------------------------------------------------------------------- */
 ...
 * =============================== */
  ...
  * Constructor just calls the base. *
  * -------------------------------- */
SimplePixelShader::SimplePixelShader(ID3D11Device* device, ID3D11DeviceContext* context) : ISimpleShader(device, context) {shader = nullptr;}
 ...
 * ================================ */
  ...
  * Constructor just calls the base. *
  * -------------------------------- */
SimpleDomainShader::SimpleDomainShader(ID3D11Device* device, ID3D11DeviceContext* context) : ISimpleShader(device, context) {shader = nullptr;}

/* ----------------------------------------------------------- *
 * Destructor                                                  *
 * Clean up actual shader (base will be called automatically). *
 * ----------------------------------------------------------- */
SimpleDomainShader::~SimpleDomainShader() {CleanUp();}
...
bool SimpleDomainShader::CreateShader(ID3DBlob* shaderBlob)
{
  ...
  //Create the shader from the blob.
  HRESULT result = device->CreateDomainShader(shaderBlob->GetBufferPointer(), shaderBlob->GetBufferSize(), nullptr, &shader);
...
/* ----------------------------------------------------------------------- *
 * Sets the domain shader and constant buffers for future DirectX drawing. *
 * ----------------------------------------------------------------------- */
void SimpleDomainShader::SetShaderAndCBs()
{
  ...
  for(unsigned int i = 0; i < constantBufferCount; ++i)
  {
    ...
    //This is a real constant buffer, so set it.
    deviceContext->DSSetConstantBuffers(constantBuffers[i].BindIndex, 1, &constantBuffers[i].ConstantBuffer);
  }
}
 ...
 * ============================== */
  ...
  * Constructor just calls the base. *
  * -------------------------------- */
SimpleHullShader::SimpleHullShader(ID3D11Device* device, ID3D11DeviceContext* context) : ISimpleShader(device, context) {shader = nullptr;}

/* ----------------------------------------------------------- *
 * Destructor                                                  *
 * Clean up actual shader (base will be called automatically). *
 * ----------------------------------------------------------- */
SimpleHullShader::~SimpleHullShader() {CleanUp();}
...
bool SimpleHullShader::CreateShader(ID3DBlob* shaderBlob)
{
  ...
  //Create the shader from the blob.
  HRESULT result = device->CreateHullShader(shaderBlob->GetBufferPointer(), shaderBlob->GetBufferSize(), nullptr, &shader);
...
/* ----------------------------------------------------------------------- *
 * Sets the hull shader and constant buffers for future DirectX drawing. *
 * ----------------------------------------------------------------------- */
void SimpleHullShader::SetShaderAndCBs()
{
  ...
  for(unsigned int i = 0; i < constantBufferCount; ++i)
  {
    ...
    //This is a real constant buffer, so set it.
    deviceContext->HSSetConstantBuffers(constantBuffers[i].BindIndex, 1, &constantBuffers[i].ConstantBuffer);
  }
}
 ...
 * ================================== */
  ...
  * Constructor just calls the base. *
  * -------------------------------- */
SimpleGeometryShader::SimpleGeometryShader(ID3D11Device* device, ID3D11DeviceContext* context, bool UseStreamOut, bool AllowStreamOutRasterization) : ISimpleShader(device, context)
{
  shader = nullptr;
  useStreamOut = UseStreamOut;
  allowStreamOutRasterization = AllowStreamOutRasterization;
 ...
 * Clean up actual shader (base will be called automatically). *
 * ----------------------------------------------------------- */
SimpleGeometryShader::~SimpleGeometryShader() {CleanUp();}
...
bool SimpleGeometryShader::CreateShader(ID3DBlob* shaderBlob)
{
  ...
  //Create the shader from the blob.
  HRESULT result = device->CreateGeometryShader(shaderBlob->GetBufferPointer(), shaderBlob->GetBufferSize(), nullptr, &shader);
...
/* ---------------------------------------------------------------------------------- *
 * Creates the DirectX geometry shader and sets it up for stream output, if possible. *
 *                                                                                    *
 * Receives:                                                                          *
 * shaderBlob = The shader's compiled code                                            *
 *                                                                                    *
...
bool SimpleGeometryShader::CreateShaderWithStreamOut(ID3DBlob* shaderBlob)
{
  ...
  //Create the shader.
  HRESULT result = device->CreateGeometryShaderWithStreamOutput(shaderBlob->GetBufferPointer(),   //Shader blob pointer.
                                                                shaderBlob->GetBufferSize(),      //Shader blob size.
                                                                &soDecl[0],                       //Stream out declaration.
                                                                static_cast<UINT>(soDecl.size()), //Number of declaration entries
                                                                nullptr,                          //Buffer strides (not used - assume tightly packed?)
                                                                0,                                //No buffer strides
                                                                rast,                             //Index of the stream to rasterize (if any).
                                                                nullptr,                          //Not using class linkage
                                                                &shader);
...
/* ---------------------------------------------------------------------------------------------------------------------------- *
 * Creates a vertex buffer that is compatible with the stream output delcaration that was used to create the shader.            * 
 * This buffer will not be cleaned up (released) by the simple shader - you must clean it up yourself when you're done with it. * 
 * Immediately returns false if the shader was not created with stream output, the shader isn't valid or the determined stream  *
 * out vertex size is zero.                                                                                                     *
 *                                                                                                                              *
 * Receives:                                                                                                                    *
 * buffer = Pointer to an "ID3D11Buffer"-pointer to hold the buffer reference.                                                  *
 * vertexCount = Amount of vertices the buffer should hold.                                                                     *
 *                                                                                                                              *
 * Returns: True if buffer is created successfully AND stream output was used to create the shader, false otherwise.            *
 * ---------------------------------------------------------------------------------------------------------------------------- */
bool SimpleGeometryShader::CreateCompatibleStreamOutBuffer(ID3D11Buffer** buffer, int vertexCount)
{
  ...
  if(!useStreamOut || !shaderValid || streamOutVertexSize == 0)
    return false;
...
void SimpleGeometryShader::UnbindStreamOutStage(ID3D11DeviceContext* deviceContext)
{
  ...
  ID3D11Buffer* unset[1] = {nullptr};
  deviceContext->SOSetTargets(1, unset, &offset);
}

/* ------------------------------------------------------------------------- *
 * Sets the geometry shader and constant buffers for future DirectX drawing. *
 * ------------------------------------------------------------------------- */
void SimpleGeometryShader::SetShaderAndCBs()
{
  ...
  for(unsigned int i = 0; i < constantBufferCount; ++i)
  {
    ...
    //This is a real constant buffer, so set it.
    deviceContext->GSSetConstantBuffers(constantBuffers[i].BindIndex, 1, &constantBuffers[i].ConstantBuffer);
  }
}
...
unsigned int SimpleGeometryShader::CalcComponentCount(unsigned int mask)
{
  ...
  result += static_cast<UINT>((mask & 1) == 1);
  result += static_cast<UINT>((mask & 2) == 2);
  result += static_cast<UINT>((mask & 4) == 4);
  result += static_cast<UINT>((mask & 8) == 8);
  return result;
}
 ...
 * ================================= */
  ...
  * Constructor just calls the base. *
  * -------------------------------- */
SimpleComputeShader::SimpleComputeShader(ID3D11Device* device, ID3D11DeviceContext* context) : ISimpleShader(device, context)
{
  shader = nullptr;
  threadsX = 0;
  threadsY = 0;
 ...
 * Clean up actual shader (base will be called automatically). *
 * ----------------------------------------------------------- */
SimpleComputeShader::~SimpleComputeShader() {CleanUp();}
...
bool SimpleComputeShader::CreateShader(ID3DBlob* shaderBlob)
{
  ...
  //Create the shader from the blob.
  HRESULT result = device->CreateComputeShader(shaderBlob->GetBufferPointer(), shaderBlob->GetBufferSize(), 0, &shader);

  //Was the shader created correctly?
  if(result != S_OK)
    return false;

  //Set up shader reflection to get information about UAVs.
  ID3D11ShaderReflection* refl;
  D3DReflect(shaderBlob->GetBufferPointer(), shaderBlob->GetBufferSize(), IID_ID3D11ShaderReflection, (void**)& refl);
  ...
  threadsTotal = refl->GetThreadGroupSize(&threadsX, &threadsY, &threadsZ);
...
void SimpleComputeShader::SetShaderAndCBs()
{
  ...
  for(unsigned int i = 0; i < constantBufferCount; ++i)
  {
    ...
    //This is a real constant buffer, so set it.
    deviceContext->CSSetConstantBuffers(constantBuffers[i].BindIndex, 1, &constantBuffers[i].ConstantBuffer);
  }
}

/* ------------------------------------------------------------------------------------------------------------------ *
 * Dispatches the compute shader with the specified amount of groups, using the number of threads per group specified *
 * in the shader file itself.                                                                                         *
 *                                                                                                                    *            
 * For example, calling this method with parameters (5, 1, 1) on a shader with (8, 2, 2) threads per group will       *
 * launch a total of 160 threads: ((5 * 8) * (1 * 2) * (1 * 2)).                                                      *
 *                                                                                                                    *
 * This is identical to using the device context's "Dispatch()"-method yourself.                                      *  
 *                                                                                                                    *
 * NOTE: This will dispatch the currently active shader, not necessarily THIS shader. Be sure to activate this shader *
 *       with "SetShader()" before calling "Dispatch()".                                                              *
 *                                                                                                                    *
 * Receives:                                                                                                          *                                                                               *
 ...
 * ------------------------------------------------------------------------------------------------------------------ */
void SimpleComputeShader::DispatchByGroups(unsigned int groupsX, unsigned int groupsY, unsigned int groupsZ) {deviceContext->Dispatch(groupsX, groupsY, groupsZ);}

/* ------------------------------------------------------------------------------------------------------------- *
 * Dispatches the compute shader with AT LEAST the specified amount of threads, calculating the number of groups * 
 * to dispatch using the number of threads per group specified in the shader file itself.                        *
 *                                                                                                               *
 * For example, calling this method with parameters (10, 3, 3) on a shader with (5, 2 ,2) threads per group      *
 * will launch eight total groups and 160 total threads, calculated by:                                          *
 * Groups: ceil(10/5) * ceil(3/2) * ceil(3/2) = 8                                                                *
 * Threads: ((2 * 5) * (2 * 2) * (2 * 2)) = 160                                                                  *
 *                                                                                                               *
 * NOTE: This will dispatch the currently active shader, not necessarily THIS shader. Be sure to activate this   *
...
{
  deviceContext->Dispatch(max(static_cast<UINT>(ceil(static_cast<float>(ThreadsX) / threadsX)), 1),
                          max(static_cast<UINT>(ceil(static_cast<float>(ThreadsY) / threadsY)), 1),
                          max(static_cast<UINT>(ceil(static_cast<float>(ThreadsZ) / threadsZ)), 1));
}
...
{
  //Look for the key.
  std::unordered_map<std::string, unsigned int>::iterator result = uavTable.find(name);
